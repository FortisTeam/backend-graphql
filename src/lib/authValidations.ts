...
import { globalVariablesConfig } from "../config/appConfig";
...
export const authValidations = {
   ...
   */
  ensureLimitOfUsersIsNotReached: (numberOfCurrentlyUsersRegistered: any) => {
   ...
   */
  ensureThatUserIsLogged: (context: any) => {
    if (!context.user) {
   ...
   */
  ensureThatUserIsAdministrator: (context: any) => {
  ...
  getUser: async (context: any) => {
    ...
    const user = await models.Users.findOne({ _id: userUUID });
    if (!user) {
      throw new AuthenticationError(
        "You must be logged in to perform this action"
      );
